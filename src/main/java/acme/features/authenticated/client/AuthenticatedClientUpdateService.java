...
public class AuthenticatedClientUpdateService extends AbstractService<Authenticated, Client> {
	...
	@Autowired
	private AuthenticatedClientRepository authenticatedClientRepository;
	...
	public void load() {
		...
		object = this.authenticatedClientRepository.findClientByUserAccountId(userAccountId);
	...
	public void validate(final Client object) {
		...
		if (!super.getBuffer().getErrors().hasErrors("identification")) {
			...
			existing = this.authenticatedClientRepository.findClientByIdentification(object.getIdentification());
	...
	public void perform(final Client object) {
		assert object != null;
		this.authenticatedClientRepository.save(object);
	}
